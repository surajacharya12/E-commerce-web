    ...
    <div className={`contact-info ${className}`}>
      ...
      {showAddress && (
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-gray-700">{contactInfo.address.full}</span>
        </div>
      )}
      ...
      {showHours && (
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="text-gray-700">
            <div>Mon-Fri: {contactInfo.businessHours.weekdays}</div>
            <div>Sat: {contactInfo.businessHours.saturday}</div>
            <div>Sun: {contactInfo.businessHours.sunday}</div>
          </div>
        </div>
      )}
    </div>
...
export function PhoneNumber({ className = "" }) {
  return (
    <a
      href={contactInfo.phone.link}
...
export function EmailAddress({ className = "" }) {
  return (
    <a
      href={contactInfo.email.link}
      className={`inline-flex items-center gap-1 hover:text-blue-600 transition-colors ${className}`}
...
export function WhatsAppButton({
  ...
  const whatsappLink = `${contactInfo.phone.whatsapp}?text=${encodeURIComponent(
    message
  )}`;
